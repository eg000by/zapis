...
//
// Почему три часа, а не один. GitHub душит кроны публичных репозиториев: наблюдаемые
// промежутки между прогонами доходят до 3 часов (каждый прогон изнутри пингует 50
// минут, то есть реально не покрыто до ~2 ч 10 мин подряд). С часовым окном занятие,
// чьё окно целиком попало в такой промежуток, оставалось вообще без уведомления —
// ровно это и случилось 30 июля с занятием в 10:10. Три часа перекрывают самый
// длинный наблюдавшийся простой.
//
// Раннее срабатывание безвредно: в тексте нет «через N минут» — только время начала,
// которое верно всегда. Точность вместо запаса даст только внешний пингер
// (см. шапку .github/workflows/pulse.yml).
export const UPCOMING_LEAD_MINUTES = 180;
